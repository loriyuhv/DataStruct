...
对于有向图G=（V,{E}），如果弧<v,v‘>属于E，则称顶点v邻接到顶点v‘，顶点v’邻接自顶点v。弧<v,v‘>和顶点v，v’相关联。以顶点v为头的弧的数目称为v的入度（```InDegree```），记为ID(v)；以v为尾的弧的数目称为v的出度（```OutDegree```），记为OD（v）；顶点v 的度为TD(v) = ID（v）+OD（v）。
